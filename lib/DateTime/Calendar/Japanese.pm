...
use DateTime;
use DateTime::Event::Chinese qw(chinese_new_year_for_gregorian_year);
...
sub _era2cycle
{
    ...
    my $ny_in_year = chinese_new_year_for_gregorian_year($era->start);
    ...
    my $cycle_year    = # amod
        $elapsed_years - 60 * ( POSIX::floor($elapsed_years / 60) ) || 60;
...
sub _calc_era_components
{
    ...
    if ($era) {
        my $midday = $self->{gregorian}->clone;
        # truncate to mid-day
        $midday->truncate(to => 'hour');
        $midday->set( hour => 12 );

        my $ny_this_gy = chinese_new_year_for_gregorian_year($midday);
        my $ny_start_gy = chinese_new_year_for_gregorian_year($era->start);

        $ny_this_gy->truncate(to => 'hour');
        $ny_this_gy->set(hour => 12);
        $ny_start_gy->truncate(to => 'hour');
        $ny_start_gy->set(hour => 12);

# print STDERR "ERA COMPONENT (0) ", $midday->year, " ", $era->start->year, "\n";
        my $year = $midday->year() - $era->start->year();
# print STDERR "ERA COMPONENT (1) $year\n";
# print STDERR "ERA COMPONENT (1.2) " . $era->start . "\n";

        # this date is before new year
# print STDERR "ERA COMPONENT (1.5) $ny_this_gy, $ny_start_gy, $midday\n";
        if ($ny_this_gy->year != $ny_start_gy->year && $ny_this_gy <= $midday) {
            $year++;
        }
# print STDERR "ERA COMPONENT (2) $year\n";
...
# print STDERR "ERA COMPONENT (3) $year\n";
